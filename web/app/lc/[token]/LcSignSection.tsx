...
import type { LaborContractView } from '@/components/PaperLaborContract';
import { CheckCircle, Pen, Download } from '@/components/Icons';
import { createT, type Lang } from '@/lib/i18n';

/** 외부(미가입) 근로자 서명 섹션 — 미서명이면 서명 폼, 서명 후 완료+가입 유도. */
export default function LcSignSection({
  token,
  initialSigned,
  view,
  lang = 'ko',
  ...
  }>({ signerName: view.workerSignerName, signedAt: view.workerSignedAt });

  const pdfHref = absoluteUrl(`/api/public/contracts/${token}/pdf`);
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/public/contracts/${token}/sign`, {
        signerName: signerName.trim(),
        signImageBase64: dataUrl,
      });
      const data = (res.data?.data ?? res.data) as {
        workerSignerName?: string;
        workerSignedAt?: string;
      };
      setSignedInfo({
        signerName: data.workerSignerName ?? signerName.trim(),
        signedAt: data.workerSignedAt,
      });
      setDone(true);
    } catch (e) {
      const msg =
